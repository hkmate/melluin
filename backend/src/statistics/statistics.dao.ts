            ...
            FROM (SELECT status
                  FROM (${this.getStaticValuesFromEnum(HospitalVisitStatus)}) x(status)) s
                     LEFT OUTER JOIN (SELECT hv.status, hv.id
                                      ...
                                      WHERE hd.valid_from < ($2)::timestamptz
                                        AND hd.valid_to >= ($1)::timestamptz
                                        and hd.city = $3
                                        AND hv.datetime_from < ($2)::timestamptz
                                        AND hv.datetime_to >= ($1)::timestamptz) hv ON hv.status = s.status
            GROUP BY s.status
        `, [from, to, city]);
    }
